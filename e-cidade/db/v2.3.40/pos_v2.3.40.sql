insert into db_versao (db30_codver, db30_codversao, db30_codrelease, db30_data, db30_obs)  values (356, 3, 40, '2015-08-10', 'Tarefas: 98370, 98371, 98372, 98376, 98377, 98378, 98379, 98381, 98382, 98383, 98384, 98385, 98386, 98388, 98389, 98390, 98393, 98394, 98398, 98399, 98401, 98403, 98404, 98405, 98406, 98407, 98408, 98410, 98411, 98412, 98414, 98415, 98416, 98417, 98418, 98419, 98420, 98421, 98422, 98423, 98424, 98425, 98427, 98429, 98432, 98433, 98436, 98438, 98439, 98443, 98445, 98446, 98447, 98448, 98449, 98450, 98451, 98452, 98453, 98455, 98456, 98457, 98458, 98460, 98461, 98462, 98463, 98464, 98465, 98467, 98468, 98469, 98471, 98472, 98473, 98474, 98475, 98476, 98479, 98480, 98483, 98484, 98485, 98487, 98488, 98489, 98491, 98493, 98494, 98495, 98496, 98497, 98498, 98500, 98502, 98503, 98506, 98508, 98510, 98512, 98513, 98514, 98517, 98519, 98520, 98521, 98523, 98524, 98525, 98526, 98527, 98528, 98532, 98534, 98535, 98536, 98541, 98543, 98544, 98546, 98548, 98549, 98553');create or replace function docentetemdisponibilidade(integer, bigint[], bigint, bigint, integer)
  returns boolean as
$$
declare
...
begin
  ...
  FOR j IN 1..array_upper(aPeriodosDisponibilidade, 1) LOOP
    ...
    select array_accum(ed58_i_periodo) 
      into aRegencias
...
as $$
 ...
 begin
    ...
    select coalesce(sum(case when m81_tipo = 1 then round(m82_quant::numeric, 5) when m81_tipo = 2 then round(m82_quant::numeric, 5) * -1  else 0 end), 0)
      into nSaldo
...
create trigger tg_saldoestoque_inc_alt before INSERT or UPDATE on matestoqueinimei for each row execute procedure fc_saldoestoque_trigger();create or replace function fc_baixabanco( cod_ret integer, datausu date) returns varchar as
$$
declare
...
$$ language 'plpgsql';create or replace function fc_iptu_taxalimpeza_guaiba_2009(integer,numeric,integer,numeric,numeric,boolean) returns boolean as
$$
declare
...
begin
    ...
    for rConstr in execute tSql loop
      ...
      if iCarTipoOcupacao = 0 then -- se ainda nao encontrou caracteristica
        ...
        loop
          ...
          if rIDConstr.j31_grupo = 45 then

            if rIDConstr.j48_caract in (30130) then
              iCarTipoOcupacao = 32; -- residencial
            elsif rIDConstr.j48_caract in (30131,30132) then
              iCarTipoOcupacao = 33; -- comercial
            elsif rIDConstr.j48_caract in (30133) then
    ...
    select j73_aliq
      into nTaxaLimpAliq
      from caraliq
     where j73_anousu = iAnousu
       and j73_caract = iTaxaLimpCar;
    ...
    perform fc_debug('bPredial:'         || bPredial, lRaise);
    perform fc_debug('iCarPavimentacao:' || iCarPavimentacao, lRaise);
    perform fc_debug('iCarMuro:'         || iCarMuro, lRaise);
    perform fc_debug('iCarTipoOcupacao:' || iCarTipoOcupacao, lRaise);
    perform fc_debug('iTaxaLimpCar:'     || iTaxaLimpCar, lRaise);
    perform fc_debug('nTaxaLimpAliq:'    || nTaxaLimpAliq, lRaise);
    perform fc_debug('nValInflator:'     || nValInflator, lRaise);
    perform fc_debug('nVlrTaxaLimpa:'    || nVlrTaxaLimp, lRaise);
    perform fc_debug('iPercIsen:'        || iPercIsen, lRaise);
...
$$ language 'plpgsql';create or replace function fc_iptu_calculavvc_guaiba_2009(integer, integer, boolean, boolean) returns tp_iptu_calculavvc as
$$
declare
    ...
    iAnoConstrucao         integer;
    iPontos                integer;
    iEfeitoLoja            integer;
    iZona                  integer;
    iInfra	  	           integer default 0;
    iInfraTer		           integer default 0;
    iInfraAnt		           integer default 0;
    lAchouInfra            boolean;
    iIdbql                 integer;
    iMult                  numeric(15,4) default 0;
    iNumerocontr           integer default 0;

    nVm2C                  numeric(15,2) default 0;

    nAreaConstruida        numeric(15,2) default 0;
    nAreaTotalConstruida   numeric(15,2) default 0;

    nVlrVenalPredial       numeric(15,2) default 0;
    nVlrVenalPredialTot    numeric(15,2) default 0;

    nVlrBase               numeric default 0;

    iCaracter              integer default 0;

    lAbastecimento         boolean default false;
    lIluminacao            boolean default false;
    lMeioFio               boolean default false;
    lPavimentacao          boolean default false;
...
begin
    ...
    for rConstr in execute tSql loop
      ...
      if iPontos is null then

        rtp_iptu_calculavvc.rtDemo    := 'sem pontuacao lancada - pontos = 0';
      ...
      loop
        ...
        if rIDConstr.j31_grupo = 32 then
          iEfeitoLoja = rIDConstr.j48_caract;
        end if;

      end loop;

      nAreaConstruida = round(rConstr.j39_area);

      if nAreaConstruida = 0 then
        nAreaConstruida = 1;
      end if;

      select j01_idbql, j34_zona
        into iIdbql, iZona
        from iptubase inner join lote on j01_idbql = j34_idbql
      ...
      loop
        ...
        if rCarLote.j31_grupo = 1 then
          ...
          iInfraTer      = iInfraTer + rCarLote.j35_caract;
          lAbastecimento = true;
        elsif rCarLote.j31_grupo = 2 then

          iInfraTer   = iInfraTer + rCarLote.j35_caract;
          lIluminacao = true;
        elsif rCarLote.j31_grupo = 3 then

          iInfraTer = iInfraTer + rCarLote.j35_caract;
          lMeioFio  = true;
        elsif rCarLote.j31_grupo = 4 then
      ...
      if lAbastecimento is false then
        ...
        if iCaracter = 193 then -- com
          iInfraTer = iInfraTer + 1;
        elsif iCaracter = 194 then -- sem
          iInfraTer = iInfraTer + 2;
        end if;
      end if;

      if lIluminacao is false then
        ...
        if iCaracter = 195 then -- com
          iInfraTer = iInfraTer + 3;
        elsif iCaracter = 196 then -- sem
          iInfraTer = iInfraTer + 4;
        end if;
      end if;

      if lMeioFio is false then
        ...
        if iCaracter = 197 then -- com
          iInfraTer = iInfraTer + 5;
        elsif iCaracter = 198 then -- sem
          iInfraTer = iInfraTer + 6;
        end if;
      end if;
      ...
      if lPavimentacao is false then
        ...
        if iCaracter in (199, 152, 153) then -- com
          iInfraTer = iInfraTer + 7;
        elsif iCaracter = 200 then -- sem
          iInfraTer = iInfraTer + 8;
        end if;
      end if;

      perform fc_debug('iInfraTer: ' || iInfraTer, lRaise);

      if iInfraTer not in (16, 18, 19, 20) then

        perform fc_debug('iInfraTer invalido: ' || iInfraTer || ' alterando para 16', lRaise);
        iInfraTer = 16;
      end if;
      ...
      select j72_valor
       ...
       where j72_anousu      =  iAnousu
         and iPontos  	     >= j72_quantini
         and iPontos         <= j72_quantfim
         and nAreaConstruida >= j72_ini
         and nAreaConstruida <= j72_fim
         and j72_tipo         = 'P'
         and j72_zona         = iZona
         and j72_caract       = iInfraTer;

      perform fc_debug('iPontos:         ' || iPontos, lRaise);
      perform fc_debug('nAreaConstruida: ' || nAreaConstruida, lRaise);
      perform fc_debug('iZona:           ' || iZona, lRaise);
      perform fc_debug('iInfraTer:       ' || iInfraTer, lRaise);
      perform fc_debug('nVm2C:           ' || nVm2C, lRaise);

      if nVm2C is null or nVm2C <= 0 then

        iInfraAnt   := iInfraTer;
        lAchouInfra := false;

        for iInfra in reverse 20..17
        loop
          ...
          if iInfraAnt = iInfra or lAchouInfra is true then
            ...
            select j72_valor
             ...
             where j72_anousu       = iAnousu
               ...
               and j72_tipo         = 'P'
               and j72_zona         = iZona
               and j72_caract       = iInfra;

            if nVm2C is not null and nVm2C > 0 then
      ...
      iMult = 1;
      if iEfeitoLoja = 143 then
        iMult = 1.3688;
      elsif iEfeitoLoja = 144 then
        iMult = 1.1699;
      end if;

      nAreaTotalConstruida := nAreaTotalConstruida + nAreaConstruida;
      nVlrVenalPredial     := round( nVm2C * nAreaConstruida * iMult * nVlrBase,2);
      nVlrVenalPredialTot  := round(nVlrVenalPredial + nVlrVenalPredialTot,2)::float8;

      perform fc_debug('iEfeitoLoja: ' || iEfeitoLoja, lRaise);
      perform fc_debug('iMult:       ' || iMult, lRaise);
      perform fc_debug('nVlrBase:    ' || nVlrBase, lRaise);

      if nVm2C is null or nVm2C = 0 then

        rtp_iptu_calculavvc.rtDemo    := 'valor m2 construcao zerado';
        rtp_iptu_calculavvc.riCodErro := 24;
        rtp_iptu_calculavvc.rbErro    := 'f';
        return rtp_iptu_calculavvc;
      end if;
      ...
      insert into tmpiptucale (anousu, matric,idcons,areaed,vm2,pontos,valor)
           values (iAnousu, iMatricula, rConstr.j39_idcons, rConstr.j39_area, nVm2C, iPontos, nVlrVenalPredial);
    ...
    perform fc_debug('Valor total venal predial : '||coalesce(nVlrVenalPredialTot,0),lRaise,false,false);

    rtp_iptu_calculavvc.rnVvc       := nVlrVenalPredialTot;
    rtp_iptu_calculavvc.rnTotarea   := nAreaTotalConstruida;
...
$$  language 'plpgsql';drop function if exists fc_iptu_taxalimpeza_sap_2008 (integer, numeric, integer, numeric, numeric, boolean);
create or replace function fc_iptu_taxalimpeza_sap_2008 (integer, numeric, integer, numeric, numeric, boolean) returns boolean as
$$
declare
    ...
    lComercio      boolean       default false;
...
begin
    ...
    perform fc_debug(' <iptu_taxalimpeza> historico: ' || iHistCalc, lRaise);
		...
		select valor
		  into nValAux
			from tmpoutrosvalores
 		 where descricao = 'limpeza';

    perform fc_debug(' <iptu_taxalimpeza> nValAux: ' || nValAux, lRaise);
    ...
    perform fc_debug(' <iptu_taxalimpeza> lComercio: ' || lComercio, lRaise);
    perform fc_debug(' <iptu_taxalimpeza> nValAux: ' || nValAux, lRaise);

	  if nValAux is not null and nValAux = 0 then
      return true;
    end if;
    ...
    -- se comercio e area construida > 100 dobra o valor da taxa de limpeza
		if nTotAreaConstr > 100::numeric and lComercio is true then
		  nValTaxa := (nValTaxa*2)::numeric;
		end if;
...
$$ language 'plpgsql';create or replace function fc_disbanco_instit_inc() returns trigger as
$$
declare

  iInstitSessao                  integer;
  iInstitNumpre                  integer;
  lPermiteNumpreOutraInstituicao boolean default false;
...
begin
  ...
  /**
   * Variavel de sessão para nao validar a instituicao do debito na inclusão
   */
  lPermiteNumpreOutraInstituicao := ( case when fc_getsession('DB_permiteNumpreOutraInstituicao') is null
                                           then false
                                           else true
                                       end );
  if lPermiteNumpreOutraInstituicao = true then

    new.instit := iInstitSessao;
    return new;
  end if;


  -- Verifica a instituicao do numpre
  select k00_instit
    into iInstitNumpre
    from arreinstit
   where k00_numpre = new.k00_numpre;

  -- Se encontrou instituicao do numpre
  if found then
    -- Verifica se instituicao do numpre eh diferente do da sessao
    if iInstitSessao <> iInstitNumpre then
      raise exception 'Débito pertencente a outra instituição';
    end if;
  end if;

  -- Atribui instituicao da sessao na tabela disbanco
	if new.instit is null and iInstitSessao is not null then
    new.instit := iInstitSessao;
  end if;
...
drop trigger "tg_disbanco_instit_inc" on disbanco;
create trigger "tg_disbanco_instit_inc" before insert on "disbanco" for each row execute procedure "fc_disbanco_instit_inc" ();--p--drop function fc_issqn(integer,date,integer,date,boolean,boolean);
...
$$ language 'plpgsql';insert into db_versaoant (db31_codver,db31_data) values (356, current_date);
...
select setval ('db_versaocpdarq_db34_codarq_seq',(select max (db34_codarq) from db_versaocpdarq));create table bkp_db_permissao_20150810_105159 as select * from db_permissao;
