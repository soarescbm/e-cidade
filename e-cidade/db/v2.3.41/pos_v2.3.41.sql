insert into db_versao (db30_codver, db30_codversao, db30_codrelease, db30_data, db30_obs)  values (357, 3, 41, '2015-09-14', 'Tarefas: 98150, 98187, 98402, 98540, 98547, 98550, 98554, 98555, 98557, 98559, 98560, 98561, 98562, 98563, 98564, 98565, 98566, 98567, 98570, 98571, 98572, 98573, 98574, 98575, 98576, 98577, 98580, 98581, 98582, 98583, 98585, 98586, 98587, 98588, 98589, 98590, 98591, 98592, 98593, 98594, 98595, 98596, 98600, 98601, 98602, 98603, 98604, 98605, 98606, 98607, 98609, 98611, 98613, 98614, 98615, 98616, 98617, 98618, 98619, 98620, 98621, 98622, 98623, 98624, 98625, 98626, 98627, 98628, 98629, 98630, 98631, 98634, 98635, 98636, 98639, 98641, 98643, 98644, 98645, 98647, 98648, 98649, 98650, 98651, 98654, 98657, 98659, 98660, 98661, 98662, 98663, 98664, 98665, 98666, 98668, 98670, 98672, 98673, 98674, 98675, 98676, 98678, 98679, 98680, 98681, 98682, 98684, 98688, 98689, 98690, 98691, 98692, 98693, 98694, 98696, 98697, 98698, 98699, 98700, 98701, 98705, 98706, 98707, 98708, 98709, 98710, 98712, 98713, 98714, 98716, 98717, 98718, 98719, 98721, 98722, 98723, 98724, 98725, 98727, 98729, 98730, 98731, 98733, 98734, 98736, 98739, 98740, 98742, 98743, 98745, 98746, 98748, 98750');drop function fc_saldoitens(integer, boolean,integer ,integer);
...
$$ language 'plpgsql';CREATE OR REPLACE FUNCTION fc_conta_dias_assentamento_periodo(date, date, integer, integer, integer, int[], int[]) RETURNS integer AS $$
DECLARE
  totalDias                   integer := 0;
  nroDias                     integer := 0;
  dataInicio                  alias for $1;
  dataFinal                   alias for $2;
  matriculaServidor           alias for $3;
  minimoDias                  alias for $4;
  diasAssentamento            alias for $5;
  tipoAssentamentos           alias for $6;
  tipoAssentamentosMinimoDias alias for $7;
BEGIN

  if diasAssentamento is not null then
    totalDias = diasAssentamento;
  end if;

  select afastamentos.periodo_afastado + ( afastamentos.periodo_afastado_saude )::integer into nroDias
    from ( select distinct h16_regist as matricula,
                  sum(case when h16_assent = ANY(tipoAssentamentos) 
                           then case when h16_dtterm is null
                                     then case when h16_dtconc < dataInicio
                                               then case when dataFinal < current_date
                                                         then (dataFinal - dataInicio)+1
                                                         else current_date - dataInicio
                                                     end
                                               else case when dataFinal < current_date
                                                         then (dataFinal - h16_dtconc)+1
                                                         else current_date - h16_dtconc
                                                     end
                                           end
                                     when h16_dtterm > dataFinal
                                     then case when h16_dtconc < dataInicio
                                               then case when dataFinal < current_date
                                                         then (dataFinal - dataInicio)
                                                         else h16_dtterm+1 - dataInicio
                                                     end
                                               else case when dataFinal < current_date
                                                         then (dataFinal - h16_dtconc)
                                                         else h16_dtterm+1 - dataInicio
                                                     end
                                            end
                                     else case when h16_dtconc < dataInicio
                                               then (h16_dtterm-dataInicio)+1
                                               else (h16_dtterm-h16_dtconc )+1
                                           end
                                 end
                           else 0
                       end ) as periodo_afastado,
                  sum(case when h16_assent = ANY(tipoAssentamentosMinimoDias)
                           then case when h16_dtterm is null
                                     then case when h16_dtconc < dataInicio
                                               then case when dataFinal < current_date
                                                         then (dataFinal - dataInicio)+1
                       ...
                       end ) as periodo_afastado_saude
             from assenta as assentamentos
            where (h16_assent      = ANY(tipoAssentamentos)
                   or  (    h16_assent = ANY(tipoAssentamentosMinimoDias)
                        and ((h16_dtterm-h16_dtconc)+1) > minimoDias
                   ))
              and h16_regist = matriculaServidor
              and (   (    (   (h16_dtconc >= dataInicio) 
                            and (h16_dtconc <= dataFinal)
                           )
                        and
                           (   (h16_dtterm is null)
                            or (h16_dtterm > dataFinal)
                            or (h16_dtterm <= dataFinal)
                           )
                      )
                   or ( h16_dtconc < dataInicio and (    h16_dtterm >= dataFinal
                                                       or (h16_dtterm <= dataFinal and h16_dtterm >= dataInicio) 
  ...
  totalDias = totalDias + nroDias;

  -- RAISE NOTICE 'Numero de dias: %', nroDias;
  -- RAISE NOTICE 'Total de dias: %', totalDias;

  RETURN totalDias;
END;
$$ LANGUAGE plpgsql;drop FUNCTION if exists fc_gts_padrao(date, date, integer, int, int, int[], int[]);
drop type if exists tp_gts;
create type tp_gts as ( diasAfastamentoTotal integer,
                        dataFinal            date
                      );

CREATE OR REPLACE FUNCTION fc_gts_padrao(date, date, integer, int, int, int[], int[]) RETURNS tp_gts AS $$
DECLARE
  condicao                    integer := 0;
  diasAfastamento             integer := 0;
  dataInicio                  alias for $1;
  dataFinal                   alias for $2;
  matriculaServidor           alias for $3;
  minimoDias                  alias for $4;
  ...
  tipoAssentamentos           alias for $6;
  tipoAssentamentosMinimoDias alias for $7;
  rtp_gts                     tp_gts%ROWTYPE;
BEGIN

  rtp_gts.diasAfastamentoTotal := 0;

  select fc_conta_dias_assentamento_periodo(dataInicio, dataFinal, matriculaServidor, minimoDias, diasAssentamento, tipoAssentamentos, tipoAssentamentosMinimoDias) into diasAfastamento;
  
  if diasAfastamento > 0 then
    rtp_gts.diasAfastamentoTotal := rtp_gts.diasAfastamentoTotal + diasAfastamento;
  end if;
  
  -- RAISE NOTICE 'dataInicio: %', dataInicio;
  -- RAISE NOTICE 'dataFinal: %', dataFinal;
  -- RAISE NOTICE 'Total de dias: %', rtp_gts.diasAfastamentoTotal;
  -- RAISE NOTICE 'Total de diass: %', diasAfastamento;

  while (diasAfastamento > 0) loop

    dataInicio := dataFinal;
    dataFinal  := dataFinal + diasAfastamento;
    
    select fc_conta_dias_assentamento_periodo(dataInicio, dataFinal, matriculaServidor, minimoDias, 0, tipoAssentamentos, tipoAssentamentosMinimoDias) into diasAfastamento;
    
    if diasAfastamento > 0 then
      rtp_gts.diasAfastamentoTotal := rtp_gts.diasAfastamentoTotal + diasAfastamento;
    end if;

    rtp_gts.dataFinal            := dataFinal;
...
$$ language 'plpgsql';drop function if exists fc_calculoiptu_ale_2009(integer,integer,boolean,boolean,boolean,boolean,boolean,integer,integer);
...
begin
  ...
  lSulRaise := ( fc_getsession('DB_debugon') = 'false' );
...
/* VERIFICA SE OS PARAMETROS PASSADOS ESTAO CORRETOS */
  ...
  if iNumconstr is not null and iNumconstr > 0 then
      select fc_iptu_getaliquota_ale_2009(iMatricula,iIdbql,iNumcgm,true,lSulRaise) into nAliquota;
  else
      select fc_iptu_getaliquota_ale_2009(iMatricula,iIdbql,iNumcgm,false,lSulRaise) into nAliquota;
  end if;
...
--  raise notice 'aaa - nVvc: % - nVvt: %', nVvc, nVvt;
...
$$  language 'plpgsql';/*create type tp_iptu_calculavvt as (rnVvt     numeric(15,2),
...
create or replace function fc_iptu_calculavvt_ale_2009(integer,integer,integer,numeric,numeric,boolean,boolean) returns tp_iptu_calculavvt as
$$
declare
    ...
    iCaractGrupo19   integer;
    iCaractGrupo20   integer;

    nMultEsquina     numeric default 1;
    nMultPedologia   numeric default 1;
    nMultTopografia  numeric default 1;

    nAreaTotLote     numeric;
    nAreaReallote    numeric;
    nValorM2tZona    numeric;
    nVlrBase         numeric;
    nValorTerreno    numeric;


    nAreaLoteIsento   numeric;
    nProfund          numeric;
    nTestada          numeric;
    iFace             integer default 0;
    nAreaCorrigida    numeric;
    nVlrM2Terreno     numeric;
    nVlrVenalTerreno  numeric;
...
-- calcula a profundidade media
    ...
    from fc_iptu_verificaisencoes(iMatricula,iAnousu,bMostrademo,lRaise);

    IF nAreaLoteIsento > 0 THEN
      perform fc_debug('AREA REAL DO LOTE: ' || nAreaLote,lRaise,false,false);
      nAreaRealLote = nAreaLote - nAreaLoteIsento;
      IF nAreaRealLote < 0 THEN
        rtp_iptu_calculavvt.rnVvt     := 0;
        rtp_iptu_calculavvt.rnArea    := 0;
        rtp_iptu_calculavvt.rtDemo    := '';
        rtp_iptu_calculavvt.rtMsgerro := 'Area real do lote nao pode ser menor que 0 (zero)';
        rtp_iptu_calculavvt.rbErro    := 't';
        return rtp_iptu_calculavvt;
      END IF;
      perform fc_debug('AREA ISENTA DO LOTE: ' || nAreaLoteIsento,lRaise,false,false);
    else
      nAreaRealLote = nAreaLote;
    END IF;

    SELECT J37_FACE, J81_VALORTERRENO, CASE WHEN J36_TESTLE = 0 THEN J36_TESTAD ELSE J36_TESTLE END AS J36_TESTLE
    INTO iFace, nVlrM2Terreno, nTestada
    FROM IPTUCONSTR
    ...
    WHERE J39_MATRIC = iMatricula AND J39_DTDEMO IS NULL AND J01_BAIXA IS NULL LIMIT 1;

    IF iFace IS NULL THEN

      SELECT J49_FACE, J81_VALORTERRENO, CASE WHEN J36_TESTLE = 0 THEN J36_TESTAD ELSE J36_TESTLE END AS J36_TESTLE
      INTO iFace, nVlrM2Terreno, nTestada
      FROM TESTPRI 
    ...
    IF iFace is null or iFace = 0 THEN
      rtp_iptu_calculavvt.rnVvt     := 0;
      rtp_iptu_calculavvt.rnArea    := 0;
      rtp_iptu_calculavvt.rtDemo    := '';
      rtp_iptu_calculavvt.rtMsgerro := 'testada principal do lote nao cadastrada';
      rtp_iptu_calculavvt.rbErro    := 't';
      return rtp_iptu_calculavvt;
    END IF;

    nProfund := nAreaRealLote / nTestada;

    nAreaCorrigida := SQRT(nProfund);

    perform fc_debug('AREA DO LOTE UTILIZADA PARA CALCULO: ' || nAreaRealLote,lRaise,false,false);
    perform fc_debug('TESTADA TRIBUTADA: ' || nTestada || ' METROS',lRaise,false,false);
    perform fc_debug('PROFUNDIDADE (AREA DO LOTE / TESTADA): ' || nProfund,lRaise,false,false);
    perform fc_debug('AREA CORRIG BRUTA (RAIZ QUADRADA DA PROFUNDIDADE): ' || nAreaCorrigida,lRaise,false,false);

    nAreaCorrigida := nTestada * nAreaCorrigida * 5;

    perform fc_debug('AREA CORRIG LIQUIDA (TESTADA * AREA CORRIG BRUTA * 5): ' || nAreaCorrigida,lRaise,false,false);
    perform fc_debug('PROF: ' || nProfund || ' - AREA CORRIG: ' || nAreaCorrigida,lRaise,false,false);

    nVlrVenalTerreno := nAreaRealLote * (nFracao / 100) * nVlrM2Terreno;

    perform fc_debug('VALOR VENAL TERRENO: ' || nVlrVenalTerreno || ' - FRACAO: ' || nFracao,lRaise,false,false);
    perform fc_debug('Valor m2 do terreno encontrado : '||coalesce(nVlrM2Terreno,0),lRaise,false,false);

    if not found or nVlrM2Terreno = 0 then
      rtp_iptu_calculavvt.rnVvt     := 0;
      rtp_iptu_calculavvt.rnArea    := 0;
      rtp_iptu_calculavvt.rtDemo    := '';
      rtp_iptu_calculavvt.rtMsgerro := 'Valor do m2 do terreno nao configurado';
      rtp_iptu_calculavvt.rbErro    := 't';
      return rtp_iptu_calculavvt;
    end if;
    ...
    if not found or nVlrBase = 0 then
      rtp_iptu_calculavvt.rnVvt     := 0;
      rtp_iptu_calculavvt.rnArea    := 0;
      rtp_iptu_calculavvt.rtDemo    := '';
      rtp_iptu_calculavvt.rtMsgerro := 'Valor base nao encontrado para o exercio atual Exercicio : '||iAnousu;
      rtp_iptu_calculavvt.rbErro    := 't';
      return rtp_iptu_calculavvt;
    end if;

    nAreaTotLote := ( nAreaReallote * (nFracao / 100) );
    ...
    update tmpdadosiptu set vvt = rtp_iptu_calculavvt.rnVvt, vm2t=nVlrM2Terreno, areat=nAreaTotLote;
...
-- carazinho usa 30 porcento para cada atividade excedente...
    ...
    perform fc_debug('Verificando se calcula fixo ou variavel ',lRaise,false,false);
...
$$ language 'plpgsql';insert into db_versaoant (db31_codver,db31_data) values (357, current_date);
...
select setval ('db_versaocpdarq_db34_codarq_seq',(select max (db34_codarq) from db_versaocpdarq));create table bkp_db_permissao_20150914_110016 as select * from db_permissao;
